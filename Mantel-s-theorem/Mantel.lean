...
-- part 2
-- ∑_v ∑_{w∈N(v)} deg(v) = ∑ deg²
lemma sum_sum_deg_self :
    ∑ v : V, ∑ _w ∈ G.neighborFinset v, G.degree v = ∑ v : V, G.degree v ^ 2 := by
  simp_rw [sum_const, smul_eq_mul, card_neighborFinset_eq_degree]
  exact Finset.sum_congr rfl (fun v _ => by ring)

-- ∑_v ∑_{w∈N(v)} deg(w) = ∑ deg²  (对邻接对称性做交换)
lemma sum_sum_deg_swap :
    ∑ v : V, ∑ w ∈ G.neighborFinset v, G.degree w = ∑ v : V, G.degree v ^ 2 := by
  rw [Finset.sum_comm' (t' := (univ : Finset V)) (s' := fun w => G.neighborFinset w)
        (fun x y => by simp [mem_neighborFinset, adj_comm])]
  simp_rw [sum_const, smul_eq_mul, card_neighborFinset_eq_degree]
  exact Finset.sum_congr rfl (fun v _ => by ring)

-- ∑ deg² ≤ n·|E|
lemma sum_deg_sq_le (hG : G.CliqueFree 3) :
    ∑ v : V, G.degree v ^ 2 ≤ card V * G.edgeFinset.card := by
  have h_add : 2 * ∑ v : V, G.degree v ^ 2 =
      ∑ v : V, ∑ w ∈ G.neighborFinset v, (G.degree v + G.degree w) := by
    simp_rw [Finset.sum_add_distrib]
    rw [sum_sum_deg_self, sum_sum_deg_swap]
    ring
  ...
  have h_bound : ∑ v : V, ∑ _w ∈ G.neighborFinset v, card V =
    ...
    simp_rw [sum_const, smul_eq_mul, card_neighborFinset_eq_degree]
    rw [← sum_mul, sum_degrees_eq_twice_card_edges]
    ring
